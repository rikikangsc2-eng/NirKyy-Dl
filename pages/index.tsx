...
export default function Home() {
  ...
  const [isChangingEndpoint, setIsChangingEndpoint] = useState<boolean>(false);

  useEffect(() => {
    const fetchDocs = async () => {
      try {
        const res = await fetch('/api/docs');
        const data: ApiDocs = await res.json();
        setDocs(data);
        setIsFadingOut(true);
  ...
  const handleSelectEndpoint = (endpoint: ApiEndpoint, isInitial: boolean = false) => {
    if (!endpoint) return;
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      const options: RequestInit = { method: actualMethod };
